...
Contents skills that each open a browser their own way duplicate profile copies, CDP setup and SSO workarounds, which drift apart and break. How should skills and agents get a signed-in browser, and where do browser fixes live?

## Decision Outcome

**Single foundation skill with CDP attach**

Every browser session MUST be opened by the open-browser-playwright foundation skill; contents skills pass their own CDP port, then attach to the printed localhost CDP endpoint and run their own scripts.

### Details

Definitions: the **foundation** is [open-browser-playwright](skills/open-browser-playwright/SKILL.md); a **contents skill** is a `get-` or `change-<system>-contents` skill that drives a browser to integrate with a system (see [agentme-edr-127](127-external-system-adapter-skills.md)); an **ad-hoc session** is a browser opened for a user or a skill without a declared port; an **attach session** is the automation client connected to the foundation's CDP endpoint.
...
- A skill that drives a browser, or activates a contents skill that does, MUST list the foundation in its `## References`.
...
Profile, SSO, window, CDP and tab workarounds that apply to more than one site MUST be made in the foundation skill or its scripts, not in a contents skill. Site-specific page handling (e.g. a site's own login button) MAY stay in the contents skill.

---

#### 05-cdp-port-declaration

A skill that drives a browser, except a `change-<system>-contents` skill, MUST declare one quoted port as `cdp-port` under `metadata:` in its SKILL.md frontmatter (e.g. `cdp-port: "9231"`).
...
- A `change-<system>-contents` skill MUST NOT declare a port; it MUST reuse the port and session of its paired `get-<system>-contents` skill.

---

#### 06-cdp-endpoint-handling

- Every foundation session MUST expose its CDP endpoint bound to `127.0.0.1` only. Contents skills MUST reject non-localhost endpoints.
...
- Contents skills MUST close only the pages they opened, including tabs the browser opens because of their actions (e.g. popups), and MUST NOT close the browser.
