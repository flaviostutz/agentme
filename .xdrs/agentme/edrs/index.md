...
- [resolve-pr-comments](principles/skills/resolve-pr-comments/SKILL.md) - **Resolve PR comments** — Helps the OWNER of a pull request work through comments left by others: fetches every comment (GitHub or Azure DevOps) from its URL, tracks them in a local file, and walks through triaging each one (reply, won't-fix, fix) at a human-chosen automation level -- from fully automatic to fully guided, with fine-grained free-text control over the mix -- while syncing back to the provider always stays its own explicitly confirmed step. A hands-on, mutating workflow to answer feedback and land fixes -- not a code-review skill. Delegates provider reads to `get-<provider>-contents` and writes to `change-<provider>-contents`. *(skill)*
- [agentme-edr-001](principles/001-deferred-work-tracking.md) - **Deferred work tracking** - Root `TODO.md` convention for tracking deferred, out-of-scope, or postponed work: entry format, numbering, lifecycle, and pre-merge cleanup
- [agentme-edr-003](principles/003-hitl-question-content.md) - **HITL question content** - Compact template for every agent question to a human: short title with context, 2-4 options with consequences, a "(recommended)" prefix, word caps, and question-UI fields filled to their limits
- [agentme-edr-005](principles/005-skill-composition.md) - **Skill composition** - Skills activate other skills only through prose steps; no cross-skill script calls
...
- [agentme-edr-127](application/127-external-system-adapter-skills.md) - **External system adapter skills** - Priority-ordered approach, two-stage confirmation before mutations, and one `get-`/`change-<system>-contents` skill pair per system with per-resource scripts
- [get-github-contents](application/skills/get-github-contents/SKILL.md) - **Get GitHub contents** — Reads GitHub PR metadata and normalized comments via the `gh` CLI; owns authentication and checkout. *(skill)*
- [change-github-contents](application/skills/change-github-contents/SKILL.md) - **Change GitHub contents** — Replies to PR comments, posts general comments and resolves review threads via `gh`, idempotent and verified by read-back. *(skill)*
- [get-azure-devops-contents](application/skills/get-azure-devops-contents/SKILL.md) - **Get Azure DevOps contents** — Reads Azure DevOps PR metadata and normalized thread comments via `az`; owns authentication and checkout. *(skill)*
- [change-azure-devops-contents](application/skills/change-azure-devops-contents/SKILL.md) - **Change Azure DevOps contents** — Replies in PR threads, posts general comments and sets thread status via `az rest`, idempotent and verified by read-back. *(skill)*
- [agentme-edr-128](application/128-browser-automation-foundation.md) - **Browser automation foundation** - Single foundation skill opens every browser; `get-<system>-contents` skills pass their own CDP port and attach to it *(includes skill: [open-browser-playwright](application/skills/open-browser-playwright/SKILL.md))*
